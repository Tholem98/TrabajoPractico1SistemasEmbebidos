...
Describir brevemente el objetivo del trabajo práctico.

> Ejemplo:
> Comprender el proceso de arranque de un microcontrolador STM32, analizando el rol del linker, el startup y la organización de memoria.
...
- `.text`: contiene el código ejecutable (funciones)
- `.data`: variables inicializadas (copiadas de FLASH a RAM)
- `.bss`: variables no inicializadas (inicializadas en 0 en runtime)
...
Modificar la ubicación de la vector table en el archivo `linker.ld`.
...
El microcontrolador no podrá encontrar correctamente el `Reset_Handler`, ya que la tabla de vectores no estará en la dirección esperada al inicio de la memoria FLASH.

---

### ⚠️ Resultado observado
...
Modificar la copia de `.data` en `startup.c`.
...
Modificar el tamaño de RAM en `linker.ld`.
...
Antes de este trabajo, se asumía que el programa comenzaba en `main()`. Sin embargo, se pudo observar que el proceso real de arranque depende de múltiples componentes:
...
- [ ] Se compiló correctamente el proyecto
- [ ] Se incluyeron capturas de evidencia
- [ ] Se analizaron las secciones de memoria
- [ ] Se documentaron las fallas
- [ ] Se redactó una conclusión clara
